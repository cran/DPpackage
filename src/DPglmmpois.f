...
c     # of arguments = 65.
c
c     Subroutine `dpglmmpois' to run a Markov chain in the  
...
c+++++fixed effects
      ...
      double precision workp1(p,p),workp2(p,p)
...
c+++++DP
      ...
      double precision newtheta(q,mpar)
      double precision prob(nsubject+mpar)
...
c++++++++++++++++++++++++++++++++++
         ...
         if(nfixed.gt.0)then
            ...
            do i=1,nrec
               ...
               mean=exp(eta)
               ytilde=eta+(dble(yij)-mean)*exp(-eta)-offset
