c=======================================================================                      
      subroutine dpdenregr(nrec,nx,nvar,nmissi,nmiss,z,missp,npred,     #8
     &                     xpred,ngrid,grid,                            #3
     &                     a0b0,k0,nuvec,m1rand,s2inv,                  #5
     &                     s2invm2,                                     #1 
     &                     psiinv2,tau,mcmc,nsave,                      #4
     &                     cpo,thetasave,meansave,denspm,denspl,densph, #6
     &                     meanfpm,meanfpl,meanfph,                     #3
     &                     alpha,m1,muclus,ncluster,                    #4
     &                     psi1,psiinv1,s1,sigmaclus,ss,ccluster,cstrt, #7
     &                     iflag,num,denom,fs,fm,                       #5
     &                     muwork,prob,seed,sigmawork,sigworkinv,theta, #6
     &                     workm1,workm2,workm3,workmh1,workmh2,workv1, #6
     &                     workv2,workv3,ywork,                         #3
     &                     iflagx,workvx,workmx,worksam)                #4
c=======================================================================                      
c     # of arguments = 64.
c
...
c     <=2.
c
c     Copyright: Alejandro Jara, 2008
c
c     Version 1.0: 
c
...
c+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      integer iflag(nvar)
      integer iflagx(nx)
      real*8 muwork(nvar),prob(nrec+2)
      real*8 s1(nvar,nvar)
...
c++++++++++ observation in cluster with only 1 element
            ...
            if(ns.eq.1)then
               ...
               do j=since+1,ncluster
                  do k=1,ccluster(j)  
                     cstrt(j-1,k)=cstrt(j,k)          
                  end do
               end do
...
c+++++++++++++ print
     ...
     &                worksam,fs,denspl,densph)
      ...
      call hpddensregmf(nsave,npred,meansave,0.05d0,1,
...
c=======================================================================      
      subroutine hpddensregmf(nsave,npred,meansave,alpha,tint,
