...
c=======================================================================                      
      subroutine dpbinaryp(model,nrec,p,sens,spec,x,yobs,
     &                     nlink,xlink,
     &                     a0b0,betapm,betapv,
...
c=======================================================================                      
c
c     Subroutine `dpbinaryp' to run a Markov chain in the  
c     semiparametric probit regression model for binary data. 
c
...
c+++++Working space
      ...
      double precision betac(p),cdfnorm
      ...
      double precision ratio,rbeta,rtnorm
...
c+++++++ updating G base on the partition defined by beta and betac
         ...
         do i=1,npoints+1
            area=0.d0
            if(i.eq.1)then
               area=alpha*cdfnorm(endp2(1),0.d0,1.d0,1,0)
             else if(i.gt.npoints)then
               area=alpha*cdfnorm(endp2(npoints),0.d0,1.d0,0,0)
             else
               area=alpha*(cdfnorm(endp2(i  ),0.d0,1.d0,1,0)-  
     &                     cdfnorm(endp2(i-1),0.d0,1.d0,1,0))
            end if
            mass(i)=mass(i)+area
         end do
            ...
            if(ns.gt.0)then
               ...
               do j=1,mrand
                  ...
                  if(dble(runif()).le.(1.d0-(area/mass(i))))then
                     ...
                     tmp1=rtnorm(0.d0,1.d0,liminf,limsup,ainf,binf)
                     vvec(j)=tmp1
                  end if
               end do
...
c=======================================================================                  
      subroutine dppredp(nsave,nrec,npred,alpha,v,lp,out,seed1,seed2)
c=======================================================================                  
      implicit none
      ...
      double precision cdelta,cparam,cdfnorm,tmp1,tmp2,rbeta
      ...
      do i=1,npred
         do j=1,nsave
            ...
            cdelta=dble(count)
            cparam=alpha(j)*cdfnorm(lp(i,j),0.d0,1.d0,1,0)
            tmp1=(cparam+cdelta)
