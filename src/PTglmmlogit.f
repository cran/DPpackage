...
c     # of arguments = 64.
c
c     Subroutine `ptglmmlogit' to run a Markov chain in a semiparametric 
...
c                       thetsave(nsave,q+nfixed+q+2*nuniq(Sigma)+1+q*q).
c
c-----------------------------------------------------------------------
c
...
c+++++Output
      double precision acrate(6),cpo(nrec,2)
      double precision randsave(nsave,q*(nsubject+1))
      double precision thetasave(nsave,q+nfixed+q+(q*(q+1))+1+q*q)

c+++++Current values of the parameters
      double precision curr(6),cpar,beta(p),b(nsubject,q)
...
c+++++Working space - Internal
      ...
      integer ihmssf
      integer iscan,isave
      integer narea,ni,nij,nscan,nu
      integer skipcount
      integer sprint
      integer yij
      double precision acrate2,cparc
      double precision dbin,detlogl,detloglc,dlnrm
...
c++++++++++ evaluating the candidate generating kernel
            ...
            call dmvn2(q,thetac,theta,propvr,logcgko,
...
cc++++++++++ START: Model based MH
cc++++++++++ generating the candidate value
c
c            do i=1,q
c               do j=1,q
c                  propvr(i,j)=tinv(i,j)
c               end do
c            end do
c         
c            do i=1,nsubject
c               do j=1,q
c                  do k=1,q
c                     propvr(j,k)=propvr(j,k)+               
c     &                      (b(i,j)-betar(j))*(b(i,k)-betar(k))                   
c                  end do
c               end do
c            end do
c
c            do i=1,q
c               do j=1,q
c                  sigmac(i,j)=tune3*propvr(i,j)
c               end do
c            end do
c
c            nu=(dble(nsubject)+nu0)*tune3
c
c            call riwishart(q,nu,sigmac,sigmainvc,workmr,workvr,
c     &                     workmhr,workmhr2,iflagr)
c
c
cc++++++++++ evaluating the candidate generating kernel
c
c            call diwishart(q,nu,sigmac,propvr,workmr1,workmr2,workvr,
c     &                     iflagr,logcgko)        
c
c            call diwishart(q,nu,sigma,propvr,workmr1,workmr2,workvr,
c     &                     iflagr,logcgkn)        
c
cc++++++++++ ENDS: Model based MH
...
c+++++++++++++ type of partition
               ...
               do i=1,q
                  do j=1,q
                     k=k+1
                     thetasave(isave,q+nfixed+q+k)=workmr2(i,j) 
                  end do
               end do   
...
c+++++++++++++ random effects variance
               ...
               do i=1,q
                  do j=i,q
                     l=l+1
                     thetasave(isave,q+nfixed+q+k+l)=workmr1(i,j)
                  end do
               end do   
