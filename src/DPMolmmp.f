...
c=======================================================================                      
      subroutine dpmolmmp(maxni,nrec,ncateg,nsubject,nfixed,p,q,subject,#8
     &                  datastr,yr,x,z,xtx,                             #7
     &                  a0b0,prec,sb,nu,tinv1,smu,psiinv,tinv2,         #8
     &                  mcmc,nsave,                                     #2  
     &                  cutoff,ncluster,ss,alpha,beta,b,betar,mu,       #8
     &                  sigma,sigmainv,mub,sigmab,sigmabinv,y,mc,       #7
...
c     # of arguments = 63.
c
c     Subroutine `dpmolmmp' to run a Markov chain in a semiparametric 
c     probit mixed effect model for ordinal data, using a Dirichlet 
...
c                       thetasave(nsave,q+nfixed+q+(q*(q+1))+2+ncateg-2)
c
c-----------------------------------------------------------------------
c
...
c                       Xt(Y-Zb), xty(p).
c        y           :  real vector giving the latent,
c                       y(nrec).
...
c+++++Output
      ...
      real*8 thetasave(nsave,q+nfixed+q+(q*(q+1))+2+ncateg-2)
...
c+++++++++++++ cutoff points
               k=(q*(q+1)/2)    
               do i=2,ncateg-1
                  thetasave(isave,q+nfixed+l+q+k+i-1)=cutoff(i)
               end do
...
c+++++++++++++ cluster information
               k=ncateg-2  
               thetasave(isave,2*q+nfixed+2*l+k+1)=ncluster
               thetasave(isave,2*q+nfixed+2*l+k+2)=alpha
