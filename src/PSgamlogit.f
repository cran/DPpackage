...
c              do i=1,jj
c                 theta(i)=b(starts(ii)+i-1)
c                 do j=1,jj
c                    ztz(i,j)=pmatrix(starts(ii)+i-1,starts(ii)+j-1)*
c     &                       1.d0/sigmab(ii) 
c                 end do
c                 zty(i)=0.d0  
c              end do
     ...
     &                          z(i,starts(ii)+l-1)/gprime
                    ...
                    zty(j)=zty(j)+z(i,starts(ii)+j-1)*ytilde/gprime
                 end do
              end do
...
c              ll=0
c              do i=starts(ii),ends(ii)
c                 ll=ll+1
c                 mm=0
c                 do j=starts(ii),ends(ii)
c                    mm=mm+1 
c                    logpriorn=logpriorn+bc(ll)* 
c     &                        pmatrix(i,j)*(1.d0/sigmab(ii))*
c     &                        bc(mm)
c                  
c                    logprioro=logprioro+theta(ll)* 
c     &                        pmatrix(i,j)*(1.d0/sigmab(ii))*
c     &                        theta(mm)
c                 end do
c              end do
...
c               do i=starts(ii),ends(ii)
c                  do j=starts(ii),ends(ii)
c                     tmp1=tmp1+b(i)*pmatrix(i,j)*b(j)
c                  end do
c               end do
...
c            call dblepr("sigmab",-1,sigmab,nsmooth) 
         end if
...
c                  tmp2=tmp2/(xreal(ngrid,i)-xreal(1,i))  
