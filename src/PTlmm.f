c=======================================================================                      
      subroutine ptlmm(datastr,maxni,nrec,nsubject,nfixed,p,q,subject,  #8 
     &                 x,xtx,y,z,                                       #4
     &                 a0b0,mu0,prec1,prec2,sb,tinv,                    #6
     &                 mcmc,nsave,                                      #2
     &                 acrate,cpo,randsave,thetasave,                   #4
     &                 curr,b,beta,betar,mu,sigma,mc,                   #7 
...
c                       1=chol,2=asymm square, and 3=symm square.
c
c-----------------------------------------------------------------------
c
c---- MCMC parameters --------------------------------------------------
...
c+++++Prior 
      ...
      real*8 aa0,ab0,a0b0(8),mu0(q),nu0,prec1(p,p),prec2(q,q)
      real*8 sb(p)
      real*8 tau1,tau2
      real*8 tinv(q,q)      
...
c+++++Output
      ...
      real*8 thetasave(nsave,q+nfixed+1+q+(q*(q+1))+1)
...
c+++++Working space - Internal
      ...
      real*8 ratio,rgamma,rtlnorm,runif,sse,tmp1,tmp2
...
c+++++++ Test         
c         call dblepr("sigma or",-1,sigma,q*q)
c         do i=1,q
c            do j=1,q
c               tmp1=0.d0
c               do k=1,q
c                  tmp1=tmp1+sigmainv(i,k)*sigmainv(j,k)            
c               end do
c               call dblepr("el",-1,tmp1,1)
c            end do
c         end do
c         return
c+++++++ Test
